...
function createI18NData(text) {
    setI18NData(text, 'zh-Hant')
    i18n_process_promise.push(setI18NData(text, 'en'))
}

function setI18NData(text, lang = 'zh-Hans') {
    if (!i18n_json[text]) {
        i18n_json[text] = {}
    }
    if (lang === 'zh-Hans') {
        i18n_json[text][lang] = text
        return text
    } else if (lang === 'zh-Hant') {
        let hant_text = opencc.convertSync(text)
        i18n_json[text][lang] = hant_text
        return hant_text
    } else if (lang === 'en') {
...
function translateToEnglish(text, times = 0) {
    return request.post({
        ...
        try {
            i18n_json[text]['en'] = res[0].translations[0].text
        } catch (e) {
            i18n_json[text]['en'] = ''
        }
        return i18n_json[text]['en']
    }).catch(err => {
        if (times >= 5) {
            let errString = 'cnbing ttranslate err : ' + err.toString()
            console.log(errString)
            // i18n_json[text]['en'] = errString
            return text
        } else {
...
function processPlainText(text) {
    ...
    return text.replace(reg, function (match, $1, $2, $3, offset, fullstring) {
        ...
        return $1 + `{{${MINA_I18N_FUNCTION_NAME}('${hanzi}', ${MINA_I18N_LANG_NAME})}}` + $3
    })
}
...
function processMinaTemplateExpression(code) {
    ...
    try {
        ...
        // logError('processMinaTemplateExpression e = ' + e.toString())
        return code
    }
}
